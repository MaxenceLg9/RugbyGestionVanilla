        ...
        foreach ($matchs as $match) { ?>
            ...
            <section class="section-card">
                ...
                <div class="forms">
                    ...
                    <a href="../controleur/gerermatch.php?type=vue&idMatch=<?= htmlspecialchars($match->getIdMatch()) ?>&csrf_token=<?= htmlspecialchars(password_hash($match->getIdMatch() . $_SESSION['csrf_token'] . "vue", PASSWORD_BCRYPT)) ?>" class="forms saisie button"><p>Voir la feuille de match</p></a>
                    <?php if($match->isValider() || JouerUnMatch::isArchiveFDM($match->getIdMatch())) {
                        ...
                        <a href="../controleur/gerermatch.php?type=modification&idMatch=<?= htmlspecialchars($match->getIdMatch()) ?>&csrf_token=<?= htmlspecialchars(password_hash($match->getIdMatch() . $_SESSION['csrf_token'] . "modification", PASSWORD_BCRYPT)) ?>" class="forms modify button"><p>Modifier le match</p></a>
                    <?php } ?>
                    <form method="post" action="../controleur/gerermatch.php">
